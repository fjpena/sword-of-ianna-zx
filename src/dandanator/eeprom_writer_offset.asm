...
;
;
; Dandare - SEPT 2017 - Adapted to compile with Pasmo
; ----------------------------------------------------------------------------------------  	

RAMADDREXEC	EQU  45000					; <---- ALL THIS MUST BE RUN FROM RAM, SELECT RAM ADDRESS TO COPY ALL THIS CODE										
...
OFFSET		EQU RAMADDREXEC-SAVESECT
...
SSTSECERASE:PUSH AF						; Save Sector Number
			...
			CALL SENDSPCMD+OFFSET	
			CALL SENDCONF+OFFSET		; Send Confirmation pulse
...
SENDSPCMD:	LD HL,DDNTRADDRCMD			; HL=0 Command (ZESARUX)
			CALL SENDNRCMD+OFFSET		; Send command 	

			INC HL						; HL=1 Data 1 (ZESARUX)
			LD A,D						; Data 1
			CALL SENDNRCMD+OFFSET		; Send Data 1

			INC HL						; HL=2 Data2 (ZESARUX)
			LD A,E						; Data 2
			JP SENDNRCMD+OFFSET			; Send Data 2
...
; ----------------------------------------------------------------------------------------
; Send Special Command to Dandanator (ROM CALLS) - Sends Command (a), Data 1 (d) and Data 2 (e)- Prepare for Pulse
; Destroys HL, B.
; Uses Extra 2 bytes on Stack
;
;   ********************  MUST BE RUN FROM RAM IF CHANGING SLOTS ********************
;
; ----------------------------------------------------------------------------------------
SENDSPCMDR:	LD HL,DDNTRADDRCMD			; HL=0 Command (ZESARUX)
